...
const ENABLED_FILE = join(DATA_DIR, 'scribe-enabled-sessions.json');
...
function loadEnabledSessions(): string[] {
  try {
    ...
    console.error('Error loading scribe enabled sessions:', err);
  }
  return [];
}
...
function saveEnabledSessions(sessions: string[]): void {
  try {
    ...
    console.error('Error saving scribe enabled sessions:', err);
  }
}

/**
 * GET - List all sessions with scribe enabled
 */
...
/**
 * POST - Enable or disable scribe for a session
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[scribe-enabled] Error:', err);
