#!/bin/bash
# Turn ON Outdoor Lights via Homestead Mobile API
# Runs at 5:00 AM via cron
...
if echo "$STATUS" | grep -q "off"; then
    log "Outdoor Lights OFF - turning ON"
    # Tap the toggle icon (right side of Outdoor Lights row)
    curl -s -X POST "$PHONE_API/screen/tap" \
        -H "Content-Type: application/json" \
        -d '{"x": 959, "y": 1066}' > /dev/null
    sleep 1
    log "SUCCESS: Turned ON Outdoor Lights"
else
    log "Outdoor Lights already ON - no action needed"
fi
